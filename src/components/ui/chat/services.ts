...
export async function sendMessage(selectedChatID: number, message : ChatMessageDTO)
...
export async function createDietaConversation(dietaID : number) 
: Promise<{ conversation_id : number } | null>{
    try {
        const response = await fetcher(`/chatbot/conversations/create/dieta/${dietaID}`,{method: "POST"})
        if (response.ok){
